...
/// @brief
/// The list of OpenCL device functions that is able to run after the load of
/// library.
///
...
ae2f_extern ae2f_SHAREDCALL
  ...
  cl_kernel Kerns[ae2fCL_eAnnKerns_LEN];
...
ae2fCL_AnnMk(cl_command_queue premade_queue, cl_context ctx, cl_uint devCount,
             const cl_device_id *devs, cl_int *reterr);
...
/// @return
static ae2f_err_t ae2fCL_AnnMkEasy(cl_int *reterr) {
  ae2f_err_t err = 0;
  cl_int er[1] = {
      0,
  };
...
#define return(n)                                                              \
  ...
  *er = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 1, &device, 0);
  ...
  if (er[0])
    return (ae2f_errGlob_NFOUND);
  ae2fCL_Ann.Q =
      clCreateCommandQueueWithProperties(ae2fCL_Ann.Ctx, device, 0, er);
  if (er[0])
    return (ae2f_errGlob_NFOUND);
